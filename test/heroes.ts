...
//         expect(res.body.data.heroes).to.be.an('array');
//         expect(res.body.data.heroes).to.have.length(5);
//       });
//   });
...
//     return chai.request(app).get('/api/v1/heroes')
//       .then(res => {
//         let Wolverine = res.body.data.heroes.find(hero => hero.name === 'Wolverine');
...
//         .get('/api/v1/heroes/1')
//         .then(res => {
//           expect(res.body.data.hero.name).to.equal('Luke Cage');
//         });
//     });
